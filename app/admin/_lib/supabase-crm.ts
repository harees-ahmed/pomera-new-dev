...
class CRMDatabase {
  ...
  async getCompanies(filters?: {
    ...
    return withErrorHandling(async () => {
      ...
      if (filters?.search) {
        query = query.or(
          `company_name.ilike.%${filters.search}%,city.ilike.%${filters.search}%,contact_email.ilike.%${filters.search}%`
        );
      }
  ...
  async getCompanyContacts(companyId: string) {
    return withErrorHandling(async () => {
      ...
      const sortedContacts = (data || []).sort((a, b) => {
        const typeA = contactTypes?.find(
          (t) => t.contact_type_name === a.contact_type
        );
        const typeB = contactTypes?.find(
          (t) => t.contact_type_name === b.contact_type
        );
      ...
      if ("type" in note && "text" in note) {
        ...
        if (processedNote.note_type && !processedNote.note_type_id) {
          ...
          const noteType = noteTypes.find(
            (t) => t.name === processedNote.note_type
          );
          if (noteType) {
