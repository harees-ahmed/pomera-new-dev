        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {dimensions.statuses.slice(0, 3).map((status, index) => {
            const count = companies.filter((c) => {
              return (
                c.company_status?.toLowerCase() === status.name.toLowerCase() ||
                c.company_status
                  ?.toLowerCase()
                  .includes(status.name.toLowerCase()) ||
                status.name
                  .toLowerCase()
                  .includes(c.company_status?.toLowerCase() || "")
              );
            }).length;
            ...
            return (
              <div key={status.name} className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center">
                  <div
                    className={`p-2 ${
                      iconColors[index]?.bg || "bg-gray-100"
                    } rounded-lg`}
                  >
                    <svg
                      className={`w-6 h-6 ${
                        iconColors[index]?.text || "text-gray-600"
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
